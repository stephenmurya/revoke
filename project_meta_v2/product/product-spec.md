...
Commitment Credits are optional, purchased only through Google Play Billing, and usable only inside Revoke. Credits may be locked behind a Commitment, released on verified success, released on unverifiable/cancelled outcomes, forfeited only after verified failure and exhausted grace, or redeemed for Premium access time. Phase 7 implements a repository-level Credit wallet, purchase/disclosure, hold/backing, evidence-upload, settlement, reversal, and redemption boundary over retained schedule infrastructure; live Play configuration and production proof remain release gates.
...
Keep non-financial Time Block and Usage Limit enforcement, local-first operation, basic Insights, optional Circle permissions, the bounded override flow, the Premium entitlement boundary, and the Phase 7 Credit compatibility boundary. Phase 5 implements Circle/authority semantics over compatibility storage, Phase 6 implements Premium over server-verified Google Play grants, and Phase 7 implements Credit accounting and backing over retained schedule infrastructure; none introduces a native/server-native Commitment domain. Defer native/server Commitment leases, complete evidence-integrity hardening, advanced analytics, community regimes, and production commercial operations until their prerequisites are complete.
