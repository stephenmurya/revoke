...
| Credit-backed Commitment activation is an atomic server hold over an existing schedule compatibility boundary | ACCEPTED | decisions/015; architecture/credit-backed-commitments.md | Repository callable, native cache, and journal bridge exist; activation is fail-closed by default until evidence is server-verifiable | Device/evidence hardening remains |
